...
func generateFile(gen *protogen.Plugin, file *protogen.File) {
	...
	protoCloneIdent := protogen.GoIdent{
		GoName:       "Clone",
	...
	process = func(messages []*protogen.Message) {
		for _, message := range messages {
			...
			p.P(`func (x *`, typeName, `) DeepCopyInto(out *`, typeName, `) {`)
			p.P(`p := `, protoCloneIdent, `(x).(*`, typeName, `)`)
			p.P(`if out == nil {`)
			p.P(`out = new(`, typeName, `)`)
			p.P(`}`)
			p.P(protoMergeIdent, `(out, p)`)
			p.P(`}`)
			...
			p.P(`func (x *`, typeName, `) DeepCopy() *`, typeName, ` {`)
			p.P(`if x == nil { return nil }`)
			p.P(`out := new(`, typeName, `)`)
			p.P(`x.DeepCopyInto(out)`)
			p.P(`return out`)
			p.P(`}`)
			...
			p.P(`func (x *`, typeName, `) DeepCopyInterface() interface{} {`)
			p.P(`return x.DeepCopy()`)
			p.P(`}`)
